...
a scalable correlator architecture that has facilitates tracking 
Moore's Law, and a real-time compression scheme that reduces data volume by a factor of $\sim$40.
PAPER pioneered the delay-spectrum foreground avoidance technique \cite{parsons_et_al2012b},
add applied it to observations (Fig. \ref{fig:eor_pspec}; \citealt{parsons_et_al2013}).
...
% As with PAPER, we plan to apply
...
The data storage and transfer plan for HERA draws heavily on the experience with PAPER data, and indeed the size and type of data generated will be extremely similar until HERA-331.  HERA will require two data storage facilities: local storage in the KAPB, and a US-based storage facility, located at Penn. The KAPB 1.5 PB system on site will house all raw data (1.2 PB) in addition to all of the compressed data products. A 500 TB system at Penn will be able to to house the various compressed data products forming the core of the analysis, as described below, and will serve these to the collaborating institutions in the US.
