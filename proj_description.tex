...
stars and black holes through 
%SF: I added a brief definition of reionization, it's never clear when that is needed
the full reionization of the primordial
intergalactic medium (IGM), when the hydrogen pervading the Universe (and that had been neutral 
since cosmological recombination) was ionized by luminous sources. 
%SF: I changed the period here becaues the first stars were probably popping up at z~30, which is ~100 Myr after
%the Big Bang
During this epoch, roughly 0.1--1~Gyr after the Big Bang, the Universe transformed from one dominated by simple,
linear evolution driven by gravity to an extraordinarily complex system with stars, black holes, and galaxies that
exerted massive influence on all the baryons in the Universe through widespread heating and reionization.
...
21~cm reionization power spectrum from $z = 7$ to 12. HERA-127 will provide tight constraints on 
...
HERA 
%SF: I don't like this "May be capable of" - sounds like waffling, which is dangerous in a proposal
%JCP: Also confusing is "in its final stages" -- makes it sound like its not HERA-331 doing the imaging
may be capable of imaging the largest structures in the primordial
...
Study of the evolution of the neutral IGM and reionization has historically been a high priority in astronomy 
and is a motivator for many future large telescopes.  Recent measurements from the {\it Hubble Space Telescope} 
...
the Dark Ages. This capability
will be transformative for
studying the imprint of the very first galaxies in the Ly-$\alpha$ and X-ray backgrounds, providing measurements on the properties of the first galaxies that
...
%JCP: the above sentence has two "ands": and how feedback from early star formation... and the integrated global ionization profile... is this a mistake? or is there some connection between these two things I don't understand? 
...
end of the dark ages.  As shown in the right-hand panel of Figure \ref{fig:x_i_vs_z}, HERA will have the low frequency coverage and
...
\caption{\small Left: HERA's power-spectrum sensitivity at the $2\sigma$ level (solid)
...
\end{figure}
%JCP: the purple line in the RHS isn't mentioned in the caption.  It's also confusing that it's in one panel but not the other.

A season of observing with HERA-127 will yield high-significance constraints on the 21 cm power 
spectrum across a wide range of k modes and redshifts \citep{pober_et_al2014}.
% JCP: I don't actually talk about HERA 127 in the paper, but the cite is OK by me 
...
a 127-element HERA array (``HERA-127") can achieve a $>~10\sigma$ detection over a broad range of redshifts.  Subsequent observing with 331 elements (``HERA-331") boosts this to $>25\sigma$.  With detailed foreground modeling techniques developed for the MWA, the overall significance rises to $90\sigma$, along with new access lower $k$ modes and therefore qualitatively different physics.  Such a high sensitivity measurement would also allow one to go beyond constraining parameters, testing rather than assuming the underlying theoretical framework.
...
Power spectrum measurements with HERA-331 are sensitive enough to place strict constraints on theoretical models of reionization.  To first order, the major features of the power spectrum can be parameterized by three terms: $\zeta$, the efficiency at which galaxies release ionizing photons 
into the IGM; $T_{\rm vir}$, the minimum virial temperature of halos that produce ionizing photons; and $R_{\rm mfp}$, the mean free path for ionizing 
photons traveling through the IGM, determined the prevalence of dense Ly-limit systems.  Current 
observations limit the value of these parameters to within an order-of-magnitude.  As shown in Figure \ref{fig:ErrorEllipses},
we expect
HERA-331 power spectrum observations to constrain these parameters to better 
than 5\% using proven foreground avoidance techniques, and even better with explicit foreground modeling \citep{pober_et_al2014}.
...
with $500\times$ the survey speed and sufficiently high sensitivity to directly image reionization.  After 100 hours on a 
...
which predict peaks of up to 400~$\mu$Jy/beam.

As was the case with the power spectrum, noise and foreground residuals are important limiting factors.  Fortunately, many of the same foreground avoidance and foreground modeling schemes used for power spectrum estimation can also be applied to imaging.  In Figure \ref{fig:imaging} we demonstrate the 
resultant sensitivity of imaging in a foreground avoidance scheme, assuming a conservative 1~MHz of effective bandwidth, or 12 Mpc along the line of 
...
HERA-331 will be able to directly image HI during reionization.  Shown here is a simulation of EoR emission \citep{mcquinn_et_al2007} as imaged by HERA in 100 hours of observation.  
Black contours enclose regions with signal to noise above 10.  
...
% JCP: the last two sentences repeat things already said in the text
}  \label{fig:imaging}
\end{figure}    
...
With high sensitivity throughout the observing band, HERA will push the redshift 
frontier of current-generation $21\,\textrm{cm}$ instruments, extending observations to the pre-reionization era ($z \sim 20$)
in an  uninterrupted way.  Doing so will allow a measurement of an earlier peak in the power 
spectrum, that arises from an era of IGM heating from various X-ray sources.  Theoretical expectations 
...
pristine HI provides important contextual information on early galaxy formation.
HI images can be cross-correlated with other diffuse 
tracers of large scale structure.  A number of studies have proposed cross-correlation of 21~cm surveys with large scale intensity
mapping of molecular  CO \citep{lidz_et_al2011} and atomic CII \citep{gong_et_al2011} lines. These studies
trace the large scale galaxy distribution -- the sources of reionization. 
Prototypes of these experiments 
may be operating on the HERA timescale. Such probes have different systematics 
...
PAPER pioneered the delay-spectrum foreground avoidance technique \cite{parsons_et_al2012b},
...
foreground systematics by at least 4 orders of magnitude, leaving residuals demonstrably noise-dominated.
...
precipitously to become thermal-noise limited.
%in the ``EoR window" (blue/black 
%region), where measurements are thermal-noise limited.  
The width of the wedge derives both from an instrument-dependent component 
...
interact with baselines of different lengths, it turns out this flexibility handle
...
for the expansion the data storage, and upgrading to a 30-node computing cluster.  This cluster will support the bulk of the analysis by HERA collaborators (see \S\ref{sec:analysis}) that requires access to the full set of HERA observations.
...
NAS devices and RAID storage, transfered by air-freight shipping, will ensure full data transfer to UPenn.
...
information about array health from all subsystems and provide an accurate
...
imaging will be incorporated into the on-site QA server that is specifically
for this purpose.
...
This is a 4 year proposal, with recurring cycles of development, testing, critical design review, deployment, commissioning, observation.  Each subsystem is tested prior to a critical design review that assesses the readiness of each subsystem.  Deployment and integration is coupled with commissioning activities that verify communication and control over defined interfaces. A more detailed scheduled is contained in the project management plan, but Table \ref{tab:scheduleSummary} gives an overview of the milestones and deliverables. The Project Management Plan provides details of milestones and design review criteria for each subsystem.
...
The HERA program will produce broader impacts in a range of areas.  At the core of our effort is a concerted international scientific collaboration and student exchange program with South Africa.  In addition, we will continue the PAPER and MWA legacy of training new scientists and engineers at the graduate, undergraduate levels, postdoctoral levels.  We will make major data products available publicly as a benefit to the community (see the Data Management Plan) and will help to define the science context and techniques for the next generation of experiments, particularly the SKA.
...
%\compress
%\subsection{Additional S}

Building on past success with the CASPER program, the Berkeley cohort will be focused primarily on digital engineering.  
...
PAPER has an admirable history of enlisting interns from South African universities as 
...
MIT has
1) We trained 7 (or more?) physics undergrads and all of those that graduated have gone on to grad school in physics;
2) We made our correlator design public to CASPER community;
3) we involved many grad students/postdocs/visiting professors outside the field in the instrument construction.

Steve Furlanetto at UCLA has led two summer REU projects, and has
...
presented his work to three astronomy clubs in the LA area and at more than a dozen astronomy and physics colloquia and invited talks. 

Aguirre at Penn has had numerous undergraduate and high school students work on PAPER data, including one who won the Acorda Scientific Excellence Award (ASEA).
...
We will also provide data products to the community.
