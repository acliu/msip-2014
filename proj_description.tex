...
will characterize the power spectrum in detail, as well as
provide images of the ionized bubbles (\S\ref{sec:science}).
}\label{fig:EoRsims} \end{figure}
...
{\bf Imaging HI}. With a nearly completely sampled aperture over 300~m across, HERA will have the collecting area of Arecibo, but
...
constrained.  HERA measurements will also be sensitive to velocity streaming
...
develop, test, and deploy DSP instrumentation for radio astronomy.  This technology
...
{\bf Calibration and Snapshot Imaging}. MIT will lead 
...
complement electromagnetic simulations.  The polarization beam models
produced will determine whether polarization leakage can be formally retired as 
...
pipeline of the MWA will be adapted for HERA by the U. Washington team.  The result will be sky maps with
full Stokes information that can be used not only to directly subtract foregrounds,
but also as an interesting data product in itself.  We plan to
perform detailed studies of the polarized sky, such as characterizing the
distribution of rotation measures.  Data from HERA, PAPER, and the MWA will be used
...
will be sufficient for an unambiguous early detection of the 21~cm
...
power-spectrum estimation, U. Washington will lead the application of 
...
to measurements in both the reionization epoch and at higher redshifts.  MIT will lead the application
...
foregrounds and reionization models to output visibilities.  UCLA will also lead
a parametrized reionization simulation effort that, coupled with the instrument
simulator, will be critical for connecting power-spectrum constraints to the estimating
...
deep imaging will aim to map HI emission during reionization (see \S\ref{sec:science}).
Initially, conservative filtering of the foreground wedge will be applied to data that
are imaged.  These results will be compared with the residuals from imaging-based
...
maps will be released for cross-correlation with other probes of reionization.
%XXX work breakdown?
