...
distances \citep{ellis_et_al2013, oesch_et_al2013}. Still, this known population falls well short of the requirements to reionize the universe, driving us to ever deeper observations with, e.g., JWST and ALMA, to reveal the fainter end of the luminosity function. 
...
\end{itemize}
\noindent
Although the existing limits are weak and model-dependent, there is a substantial tension between the CMB measurements -- which suggest that the bulk of reionization occurred at $z \sim 10$ (within the hatched box in Figure~\ref{fig:x_i_vs_z}) -- and the others, which suggest (albeit at low confidence) that reionization was still substantially neutral as late as $z \sim 7$ (with the  HI ionization fraction as high as $x_{HI} \sim 0.5$).  The two curves in the left panel of Figure~\ref{fig:x_i_vs_z} illustrate this tension: the solid curve is consistent with the CMB measurements and follows simple assumptions about galaxy formation, while the dashed {\bf CHECK THIS LABEL} curve is extrapolated from galaxy observations \citep{robertson_2013}. 
%SF: It would be nice to include the following as a caveat, we probably don't have space
%While the latter is consistent with the astrophysical constraints, matching WMAP measurements requires assuming substantial star formation to $z \sim 30$.
...
regarding stellar populations, galaxy structure, and undetected sources; and (3) the CMB provides only an integral measure of ionization back to recombination. 
...
did reionization occur, and over what timescale?  What objects dominated the process, and how were they  distributed?  What role did stellar (and other) feedbackk mechanisms play during this era?  {\it HERA provides the key measurements needed 
...
era of X-ray heating (most likely sourced by stellar remnants).  These scenarios allow a
...
overlaid.  HERA's constraints in this epoch HERA can determine the rate and density
of massive black hole formation in the early universe
\citep{pritchard_loeb2010} or identify contributions by exotic heating sources like dark matter
annihilation and the effects of exotic physics likewarm dark matter.  In some scenarios, 
...
previously reionized (such as the center of large HII bubble), recently reionized (the edge of HII bubble), or is forming from
...
mapping of molecular  CO \citep{lidz_et_al2011}, atomic CII \citep{gong_et_al2011}, and HI Ly$\alpha$ \citep{silva_et_al2013} lines. These studies
trace the large scale galaxy distribution -- the sources of reionization -- and cross-correlation 
with IGM images can break degeneracies within both data sets as well as providing clean measurements of the underlying signal (because independent systematics cancel out in the cross-correlation). Prototypes of such experiments may be operating contemporaneously with HERA. 
